...
void doP(char *cmd) { command.scalar(&motory.P_angle.P, cmd); }
void doI(char *cmd) { command.scalar(&motory.P_angle.I, cmd); }
void doD(char *cmd) { command.scalar(&motory.P_angle.D, cmd); }
...
void motor_code(void *pvParameters) // This is a task.
{
  ...
  while (1)
  {
    ...
    case 0:
      ...
      {
        if (target_anglex_temp>-25&&target_anglex_temp<25&&target_angley_temp>-25&&target_angley_temp<25)
        {
        ...
        if (target_anglex_temp>25||target_anglex_temp<-25)
        {
          target_anglex = target_anglex + pid_kunx.PID_increment(target_anglex_temp);
        }
        if (target_angley_temp>25||target_angley_temp<-25)
        {
          target_angley = target_angley + pid_kuny.PID_increment(target_angley_temp);
        }
        if (target_anglex_temp > 0)
        {
          target_anglex = target_anglex + 0.0015;
        }
        else if (target_anglex_temp < 0)
        {
          target_anglex = target_anglex - 0.0015;
        }
        else
        {
          target_anglex = target_anglex;
        }
        if (target_angley_temp > 0)
        {
          target_angley = target_angley + 0.0015;
        }
        else if (target_angley_temp < 0)
        {
          target_angley = target_angley - 0.0015;
        }
        else
        {
          target_angley = target_angley;
        }


      }
      // if (target_angley_temp > 25)
      // {
      //   target_angley_temp = 25;
      // }
      // if (target_angley_temp < -25)
      // {
      //   target_angley_temp = -25;
      // }
      // if (target_anglex_temp > 25)
      // {
      //   target_anglex_temp = 25;
      // }
      // if (target_anglex_temp < -25)
      // {
      //   target_anglex_temp = -25;
...
void motor_code_init(void)
{
  float p = 000, i = 1 / 8000.0, d = 0;

  float integral_max = 0.0035714285714286, integral_min = -0.0035714285714286;
  ...
  // motory.PID_velocity.P = 0.1f;
  // motory.PID_velocity.I = 20;
  // motory.PID_velocity.D = 0.00;
  ...
  // motorx.PID_velocity.P = 0.1f;
  // motorx.PID_velocity.I = 20;
  // motorx.PID_velocity.D = 0.000;
  ...
  motorx.init();
  motory.init();
  motorx.initFOC(5.69, CW);
  motory.initFOC(1.83, CW);
  // motorx.initFOC();
  // motory.initFOC();
  // // driverx.init();
  // motorx.sensor_direction = -1;
